...
class GefApp {
  ...
  async start() {
    // Monta a estrutura da aplicação antes de inicializar dados externos.
    // Assim, uma falha de rede/Supabase/seed nunca deixa a página apenas com o fundo.
    ...
    // A tela de login não depende da carga dos seeds locais. Renderizamos
    // primeiro para evitar uma página vazia enquanto a inicialização ocorre.
    const hasAuthenticatedUser = auth.isAuthenticated();
    ...
    try {
      ...
      console.warn('[GEF] Inicialização do banco falhou; continuando com a interface:', err);
    }
